...
// Implementation of advance API. Please note that this is a demo only.
var globalWidgetDemand = [];
var globalChannelCounter = 255;
// This value must get reset to 255 after every block->python conversion
/*
	Widget created by API will start from 200 -> 100
*/
Blockly.Blocks['blynk_widget_button'] = {
	init: function () {
		this.appendDummyInput('MAIN')
			// .appendField(new Blockly.FieldImage("https://examples.blynk.cc/logo.png",60,15),'NONE')
			.appendField('when press Button')
			.appendField(new Blockly.FieldTextInput("My Button"), 'TOPIC');
		this.appendStatementInput("CODE")
		this.appendDummyInput()
		this.setColour('#b12aa0')
		this.role = "Event";
		this.module = "network";
	}
}

Blockly.Python['blynk_widget_button'] = function (block) {
	var widgetName = block.getFieldValue('TOPIC');
	var code = Blockly.Python.statementToCode(block, 'CODE');

	// TODO: Assemble Python into code variable.
	if (code.length) {
		var widgetPin = globalChannelCounter;
		globalChannelCounter -= 1;
		globalWidgetDemand.push({
			"type": "BUTTON",
			"pin": widgetPin,
			"label": widgetName
		});
		console.error("DEMAND" , globalWidgetDemand);
		var channel = widgetPin;
		var function_name = 'Button_' + channel;
...
Blockly.Blocks['blynk_widget_superchart'] = {
	init: function () {
		this.appendValueInput('MAIN')
			.appendField('plot as')
			.appendField(new Blockly.FieldTextInput("My Chart"), 'NAME');
...
Blockly.Python['blynk_widget_superchart'] = function(block) {
	var widgetName = this.getFieldValue('NAME');
	var device = "APP" //this.getFieldValue('DEVICE');
	var data = Blockly.Python.valueToCode(block, 'MAIN', Blockly.Python.ORDER_NONE);
...
Blockly.Blocks['blynk_widget_terminal'] = {
	init: function () {
		this.appendValueInput('MAIN')
			.appendField('print on ')
			.appendField(new Blockly.FieldTextInput("My Terminal"), 'NAME');
		this.setPreviousStatement(true);
		this.setNextStatement(true);
		this.setColour('#b12aa0');
		this.role = "Set";
...
Blockly.Python['blynk_widget_terminal'] = function(block) {
	var widgetName = this.getFieldValue('NAME');
	var device = "APP" //this.getFieldValue('DEVICE');
	var data = Blockly.Python.valueToCode(block, 'MAIN', Blockly.Python.ORDER_NONE);
	if (!data.length) data = 'None';
...
Blockly.Blocks['blynk_event_vr'] = {
	init: function () {
		var GeneratedList = [];
		for (var i = 0; i < 21; i++) {
			GeneratedList.push(["V" + String(i), String(i)]);
		}
		this.appendDummyInput("MAIN")
		...
		this.setOnChange(
			function (event) {
				...
				if (event.type == 'create' || event.type == 'change' || event.type == 'move') {
					if ( (this.getRootBlock().type.indexOf('blynk_event') == -1) && (this.getRootBlock().type.indexOf('blynk_widget') == -1) )  {
